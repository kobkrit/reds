...
Search.prototype.index = function (str, id, fn) {
    ...
    keys.forEach(function (word, i) {
        for (var l = 1; l < map[word].length; l++) {
            var pre_word = map[word].substring(0,l);
            var pre_score = counts[word] - (map[word].length - l) > 0? counts[word] - (map[word].length - l) : 1;
            cmds.push(['zadd', key + ':word:' + pre_word, pre_score, id]);
            cmds.push(['zadd', key + ':object:' + id, pre_score, pre_word]);
        }
    });
